            ...
            {tabs.map((item) => (
              <Button
                ...
                className={`relative px-4 py-2 rounded-full ${
                  item === selectedTab ? "bg-blue-400 dark:bg-blue-900/50 hover:bg-blue-300 dark:hover:bg-blue-900/70 text-white hover:text-white" : ""
              ...
              >
                {selectedTab.points.map((point, idx) => (
                  <div key={idx} className="flex items-start flex-col md:flex-row gap-4 p-4 rounded-md break-keep">
                    <span className="shrink-0 w-18 py-1 bg-blue-400 dark:bg-blue-700/30 rounded-lg text-white text-center text-sm">Point {idx + 1}</span>
                    <div>
